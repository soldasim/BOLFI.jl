...
# The code is taken from the unmerged issue https://github.com/JuliaStats/Distributions.jl/pull/1806/.
...
function Distributions.sampler(d::GeneralizedChisq{T}) where T
    μ = d.μ
    nchisqsamplers = [(T(d.w[i]), Distributions.sampler(NoncentralChisq(d.ν[i], d.λ[i]))) for i in eachindex(d.w)]
    normalsampler = Distributions.sampler(Normal(zero(T), d.σ)) # zero-mean
    skipnormal = iszero(d.σ)
    GeneralizedChisqSampler(μ, nchisqsamplers, normalsampler, skipnormal)
end

function Distributions.rand(rng::AbstractRNG, s::GeneralizedChisqSampler)
    result = s.μ
    for (w, ncs) in s.nchisqsamplers
        result += w * Distributions.rand(rng, ncs)
    end
    if !s.skipnormal
        result += Distributions.rand(rng, s.normalsampler)
    end
    return result
end

Distributions.rand(rng::AbstractRNG, d::GeneralizedChisq) = Distributions.rand(rng, Distributions.sampler(d))

# cdf algorithm derived from https://github.com/abhranildas/gx2, by Abhranil Das.
function Distributions.cdf(d::GeneralizedChisq{T}, x::Real) where T
    # special cases
    if isempty(d.w)
        return Distributions.cdf(Normal(d.μ, d.σ), x)
    elseif iszero(d.σ)
        # out of support (or all weights equal to zero)
        (x < Distributions.minimum(d)) && return zero(T)
        (x ≥ Distributions.maximum(d)) && return one(T)
        # inside support
        if all(==(first(d.w)), d.w)
            nchisq = NoncentralChisq(sum(T, d.ν), sum(T, d.λ))
            (first(d.w) > zero(T)) && return Distributions.cdf(nchisq, (x - d.μ)/first(d.w))
            (first(d.w) < zero(T)) && return Distributions.ccdf(nchisq, (x - d.μ)/first(d.w))
        end
    end
    # general case
    (x == -Inf) && return zero(T)
...
function Distributions.pdf(d::GeneralizedChisq{T}, x::Real) where T
    # special cases
    if isempty(d.w)
        return Distributions.pdf(Normal(d.μ, d.σ), x)
    elseif iszero(d.σ)
        # out of support
        !Distributions.insupport(d, x) && return zero(T)
        # inside support
        if all(==(first(d.w)), d.w)
            iszero(first(d.w)) && return typemax(T) # zero weights (delta at d.μ)
            nchisq = NoncentralChisq(sum(T, d.ν), sum(T, d.λ))
            return Distributions.pdf(nchisq, (x - d.μ)/first(d.w)) / abs(first(d.w))
        end
    end
    # general case
...
Distributions.logpdf(d::GeneralizedChisq, x::Real) = log(Distributions.pdf(d, x))

function Distributions.quantile(d::GeneralizedChisq{T}, p::Real) where T
    if 0 < p < 1
        # special cases
        if isempty(d.w)
            return Distributions.quantile(Normal(d.μ, d.σ), x)
        elseif iszero(d.σ) && all(==(first(d.w)), d.w)
            iszero(first(d.w)) && return d.μ # zero weights (delta at d.μ)
            nchisq = NoncentralChisq(sum(T, d.ν), sum(T, d.λ))
            (first(d.w) > zero(T)) && return Distributions.quantile(nchisq, p)*first(d.w) + d.μ
            (first(d.w) < zero(T)) && return Distributions.cquantile(nchisq, p)*first(d.w) + d.μ
        end
        # general cases
...
function Distributions.minimum(d::GeneralizedChisq{T}) where T
    d.σ > zero(T) || any(<(zero(T)), d.w) ? typemin(T) : d.μ
end

function Distributions.maximum(d::GeneralizedChisq{T}) where T
    d.σ > zero(T) || any(>(zero(T)), d.w) ? typemax(T) : d.μ
end

Distributions.insupport(d::GeneralizedChisq, x::Real) = Distributions.minimum(d) ≤ x ≤ Distributions.maximum(d)

## Recommended functions

Distributions.mean(d::GeneralizedChisq) = d.μ + sum(d.w[i] * (d.ν[i] + d.λ[i]) for i in eachindex(d.w))
Distributions.var(d::GeneralizedChisq) = d.σ^2 + 2 * sum(d.w[i]^2 * (d.ν[i] + 2*d.λ[i]) for i in eachindex(d.w))
Distributions.cf(d::GeneralizedChisq, t) = GChisqComputations.cf_explicit(d, t)
...
module GChisqComputations
    ...
    import ..Distributions.cf, ..Distributions.cdf, ..Distributions.insupport, ..Distributions.quantile_newton
    import ..Distributions.quadgk, ..Distributions.mean, ..Distributions.var
