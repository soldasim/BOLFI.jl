...
function plot_samples(bolfi, term_cond; new_datum=nothing, display=true, noise_std_true, acquisition)
    ...
    # DIVERGENCE
    mmd = BOLFI.calculate(term_cond, bolfi)
    mmd = round(mmd; digits=6)
    
    # # MAX LIKELIHOOD STD
    # if isnothing(new_datum)
    #     max_std = nothing
    # else
    #     max_std = BOLFI.calculate(term_cond, bolfi, new_datum)
    #     max_std = round(max_std; digits=6)
    # end
    ...
    # annotate!(p7, 0., -4.5, ("max var: $max_std", :cyan))
    ...
    annotate!(p8, 0., -4.5, ("MMD: $mmd", :cyan))
    ...
    annotate!(p9, 0., -4.5, ("MMD: $mmd", :cyan))
    ...
    # annotate!(p10, 0., -4.5, ("Transport Cost: $mmd", :cyan))
