...
function plot_state_1d(bolfi, new_datum; plt, acquisition)
    boss = bolfi.problem
...
function plot_data_1d!(ax, bolfi, new_datum; plt)
    ...
    scatter!(ax, new_datum, [0.];
        label = "new datum",
    )
end

function plot_eval_grid_1d!(ax; plt)
    grid = plt.acq_grid

    # scatter!(ax, grid, zeros(length(grid));
    #     label = "eval. grid",
    # )
    vlines!(ax, grid;
        label = "eval. grid",
